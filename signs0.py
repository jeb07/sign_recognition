...
def main():
    ...
    # We convert labels to a numpy array for easier manipulation
    labels_np = np.array(labels)
    x_train, x_test, y_train_raw, y_test_raw = train_test_split(
        np.array(images), labels_np, test_size=TEST_SIZE, random_state=42 # Added random_state for reproducibility
    )
    ...
    # Convert and save images for the top 5 categories
    convert_and_save_top_n_images(x_train, y_train_raw, top_5_categories, sys.argv[1])
...
def get_top_n_categories(labels, n):
    ...
    # Get the N most common labels and return their IDs
    top_n = [label for label, count in label_counts.most_common(n)]
    return top_n


def convert_and_save_top_n_images(images, labels, top_categories, data_dir):
    """
    Converts and saves images for the specified top categories from the training set
    to .png format in separate files.
    """
    output_dir = "top_5_sign_types_png"
    os.makedirs(output_dir, exist_ok=True)
    print(f"\nConverting and saving images for top {len(top_categories)} categories to '{output_dir}'...")

    saved_counts = {cat: 0 for cat in top_categories}

    for i, category in enumerate(labels):
        if category in top_categories:
            image = images[i]
            # Create a unique filename for each image to avoid overwriting
            filename = f"category_{category}_image_{saved_counts[category]:04d}.png"
